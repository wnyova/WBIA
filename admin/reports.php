...
if (!empty($eid)) {
    ...
    $start_date = date('Y-m-01', strtotime(date("Y-m-d", mktime(0, 0, 0, $selectedMonth, 1))));
    $end_date = date('Y-m-t', strtotime(date("Y-m-d", mktime(0, 0, 0, $selectedMonth, 1))));
...
// Fetch public holidays for the selected month
$public_holidays_query = $conn->prepare("SELECT DAY(holiday_date) AS day, holiday_name FROM public_holidays WHERE MONTH(holiday_date) = ?");
$public_holidays_query->bind_param("i", $selectedMonth);
